...
class LoRAConfig:
    ...
    # LoRA scaling factor.
    alpha: float = 1.0
    # Initialization function for lora_a parameters.
    init_fn: nn.initializers.Initializer = nn.initializers.normal(stddev=0.01)
    # Initialization function for lora_b parameters.
    # Zero-init ensures LoRA branch = 0 at step 0, preserving base model behavior.
    b_init_fn: nn.initializers.Initializer = nn.initializers.zeros
...
class Einsum(nn.Module):
    ...
    def setup(self):
        ...
        if config := self.lora_config:
            ...
            self.w_b = self.param("lora_b", config.b_init_fn, shape_b)
...
class FeedForward(nn.Module):
    ...
    def setup(self):
        ...
        if self.lora_config:
            # Setup LoRA parameters.
            # TODO: simplify the init_fn API.
            self.w_gating_lora = (
                self.param("gating_einsum_lora_a", self.lora_config.init_fn, (2, self.features, self.lora_config.rank)),
                self.param(
                    "gating_einsum_lora_b", self.lora_config.b_init_fn, (2, self.lora_config.rank, self.hidden_dim)
                ),
            )
            self.w_linear_lora = (
                self.param("linear_lora_a", self.lora_config.init_fn, (self.hidden_dim, self.lora_config.rank)),
                self.param("linear_lora_b", self.lora_config.b_init_fn, (self.lora_config.rank, self.features)),
            )
