...
def test_repack_transform_preserves_router_supervision():
    transform = _transforms.RepackTransform(
        structure={
    ...
    item = {
        ...
        "router_label": np.array(2, dtype=np.int32),
        "router_class_weight": np.array(1.5, dtype=np.float32),
    }

    transformed = transform(item)

    assert transformed["router_label"] == np.array(2, dtype=np.int32)
    assert transformed["router_class_weight"] == np.array(1.5, dtype=np.float32)
