...
sys.path.append("./policy/pi05/src")
    ...
    def object_hook(dct):
        if '__numpy_array__' in dct:
            ...
            if dtype == 'bfloat16':
                ...
                return np.frombuffer(raw, dtype=ml_dtypes.bfloat16).astype(np.float32).reshape(dct['shape'])
            return np.frombuffer(raw, dtype=dtype).reshape(dct['shape'])
        return dct
...
class ModelServer:
    def __init__(self, model, host='0.0.0.0', port=None, checkpoint_id=None):
        self.model = model
        self.host = host
        self.port = port
    ...
    def _create_client_session(self):
        """Create isolated per-connection state while sharing heavyweight model weights."""
        create_session = getattr(self.model, "create_session", None)
    ...
    def _handle_client(self, client_socket):
        ...
        model_session = self._create_client_session()
        with client_socket:
            while self.running:
                try:
                    ...
                    # The client emits eval_done only after all requested episodes complete.
                    # server_eval_loop.sh uses EVAL_DONE to distinguish successful completion
                    # from a premature transport disconnect.
                    if cmd == 'eval_done':
                        print("EVAL_DONE", flush=True)
                        response = {"res": "acknowledged", "checkpoint_id": self.checkpoint_id}
                        ...
                        continue  # Wait for the client to close the connection explicitly.

                    if cmd == 'report_progress':
                        tested = obs.get("tested", 0) if obs else 0
                        total = obs.get("total", 100) if obs else 100
                        print(f"PROGRESS: {tested}/{total}", flush=True)
                        if tested >= total:
                            print("EVAL_DONE", flush=True)
                            # Keep server alive for queued client tasks.
                        response = {"res": "acknowledged", "checkpoint_id": self.checkpoint_id}
                        resp_bytes = numpy_to_json(response).encode('utf-8')
                        client_socket.sendall(len(resp_bytes).to_bytes(4, 'big'))
                        client_socket.sendall(resp_bytes)
                    ...
                    # Resolve methods on connection-local state to prevent episode state leakage
                    # between concurrent or sequential evaluation clients.
                    method = getattr(model_session, cmd, None)
                    ...
                    response = {"res": result, "checkpoint_id": self.checkpoint_id}
                    ...
                    # Model/logic error: send error response but keep connection alive
                    # so the client can continue with the next episode
                    ...
                    try:
                        ...
                        print("🔌 Client connection lost while sending error response")
                        break
...
def main(usr_args):
    ...
    host = usr_args.get('server_bind_host') or usr_args.get('host', '0.0.0.0')
    ...
    server = ModelServer(model, host=host, port=port, checkpoint_id=checkpoint_id)
...
def parse_args_and_config():
    ...
    parser.add_argument('--host', type=str, default=None, help='Host/IP to bind ModelServer to')
    parser.add_argument('--port', type=int, default=None, help='Port for ModelServer (optional)')
